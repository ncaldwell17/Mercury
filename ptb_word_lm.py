...
flags.DEFINE_integer("num_gpus", 1,
                     ...
                     "running one replica.")
flags.DEFINE_string("rnn_mode", "cudnn",
                    "The low level implementation of lstm cell: one of CUDNN, "
                    "BASIC, and BLOCK, representing cudnn_lstm, basic_lstm, "
                    "and lstm_block_cell classes.")
flags.DEFINE_string("input_type", "rounds",
                    "one of the input types: rounds, days, years.")
...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, input_):
    ...
    self.pr = tf.nn.softmax(logits, axis=-1)
  ...
  def export_ops(self, name):
    ...
    ops.update({util.with_prefix(self._name, "pr") : self.pr})
  ...
  def import_ops(self):
    ...
    self.pr = tf.get_collection_ref(util.with_prefix(self._name, "pr"))[0]
...
class DataPrep(object):
  ...
  def get_tokens(self, data_path, input_type):
    word_to_id = reader._build_vocab(os.path.join(data_path, "wiki.train.txt"))
  ...
  def tok_pos(self, data, batch_size, steps, tokens):
    data_length = len(data)
    batch_length = data_length // batch_size
    data = np.reshape(data[0: batch_size * batch_length], [batch_size, batch_length])
    ...
    while True:
        ...
        for m in range(batch_size):
            for n in range(steps):
                if data[m][i + n] in tokens:
                    positions.append((m, n, data[m][i + n]))

        if i == batch_length - steps - 1:
            i = 1
        else:
            i += 1
...
def run_epoch(session, model, eval_op=None, verbose=False):
  ...
  data_prep = DataPrep()
  tokens = data_prep.get_tokens(FLAGS.data_path, FLAGS.input_type)
  token_pos = data_prep.tok_pos(model._input.data, model.batch_size, model.num_steps, tokens)
  ...
  for step in range(model.input.epoch_size):
    ...
    positions = next(token_pos)
    if len(positions) > 0:
      probs_ = []
      for position in positions:
        probs_.append(pr[position[0]][position[1]][position[2]])
      sigma += np.sum(np.log(np.array(probs_)))
      n += len(probs_)

    batches_total += (step + 1)
